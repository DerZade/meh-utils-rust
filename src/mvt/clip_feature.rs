...
mod tests {
    ...
    fn multipoly_to_rect<T: ClipFloat>(
        multi_polygon: &MultiPolygon<T>,
        idx: usize,
    ) -> anyhow::Result<Rect<T>> {
        let first_poly: &Polygon<T> = multi_polygon
            .0
            .get(idx)
            .ok_or(Error::msg("multipolygon is empty"))?;

        if !first_poly.interiors().is_empty() {
            ...
            if exterior.0.len() != 5 {
                ...
                let c_max: Coordinate<T> = Coordinate {
                    x: f32::MAX.into(),
                    y: f32::MAX.into(),
                };
                let min = exterior
                    .0
                    .iter()
                    .fold(c_max, |a: Coordinate<T>, b: &Coordinate<T>| Coordinate {
                        x: a.x.min(b.x),
                        y: a.y.min(b.y),
                    });
                let c_min: Coordinate<T> = Coordinate {
                    x: (-99999999999.9).into(),
                    y: (-999999999.9).into(),
                };
                let max = exterior
                    .0
                    .iter()
                    .fold(c_min, |a: Coordinate<T>, b: &Coordinate<T>| Coordinate {
                        x: a.x.max(b.x),
                        y: a.y.max(b.y),
                    });
    ...
    #[case(0.0, 10.0)]
    #[case(10.0, 0.0)]
    fn clip_polygon_thats_outside_the_box_will_return_none(
        #[case] offset_x: f64,
        #[case] offset_y: f64,
    ) {
        let rect = box_0_0_to_5_10();
    ...
    fn clip_multipolygon_clips_them_all() {
        ...
        match clipped {
            Some(geo::Geometry::MultiPolygon(mpg)) => {
                ...
                assert_eq!(
                    multipoly_to_rect(&mpg, 0).unwrap(),
                    polygon_partially_in_box_clipped()
                );
...
impl<T: ClipFloat> Clip<T> for Line<T> {
    ...
    fn clip(&self, rect: &Rect<T>) -> Option<Self::Output> {
        ...
        let (collinears, single_points): (Vec<LineIntersection<T>>, Vec<LineIntersection<T>>) =
            intersections.into_iter().partition(|i| match i {
        ...
        if let Some(LineIntersection::Collinear { intersection }) = collinears.first() {
            ...
            let single_point_coordinates: Vec<Coordinate<T>> = single_points
                .into_iter()
            ...
            match (
                single_point_coordinates.get(0),
                single_point_coordinates.get(1),
            ) {
                (None, None) => {
...
impl<T: ClipFloat> Clip<T> for Polygon<T> {
    ...
    fn clip(&self, rect: &Rect<T>) -> Option<Self::Output> {
        ...
        if clipped.0.is_empty() {
            ...
            Some(clipped.map_coords(|(a, b)| (T::fromf64(a), T::fromf64(b))))
        }
    }
}
