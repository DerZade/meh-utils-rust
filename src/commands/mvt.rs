...
use crate::mvt::{
    build_mounts, find_lod_layers, load_geo_jsons, ArmaMaxLodTileProjection, Collections,
    LayerSettingsFile, LodProjection, MvtGeoFloatType,
};
...
mod tests {
    use crate::commands::mvt::{
        build_contours, build_lod_vector_tiles, build_vector_tiles, calc_max_lod, create_tile,
        fill_contour_layers, try_from_geojson_feature_for_crate_feature,
        try_from_geojson_value_for_geo_geometry, vec_f64_to_coordinate_f32, MapboxVectorTiles,
    };
    ...
    fn bails_on_input_dir_empty() {
        with_input_and_output_paths(|input_path, output_path| {
            let result = (MapboxVectorTiles::new(Box::new(DummyMetaJsonParser { succeeds: true })))
                .exec(&input_path, &output_path);
    ...
    fn build_contours_creates_empty_contour_layers_for_5_10_50_100() {
        ...
        vec!["contours/05", "contours/10", "contours/50", "contours/100"]
            .into_iter()
            .for_each(|k| {
    ...
    fn build_contours_does_its_thing() {
        let contour_line_to_vec_of_tuple = |feature: &CrateFeature| -> Vec<(f32, f32)> {
            match &feature.geometry {
                geo::Geometry::MultiPolygon(empty_poly) if empty_poly.0.is_empty() => vec![],
                geo::Geometry::MultiPolygon(some_poly) => {
                    let poly = some_poly.0.get(0).unwrap();
                    let ext = poly.exterior();
                    ext.0.iter().map(|f| (f.x, f.y)).collect()
                }
                _ => vec![],
            }
        };
        ...
        let raster = DEMRaster::new(
            ...
            -9999.99,
            vec![
                0.0, 0.5, 0.5, 0.0, 0.0, 1.0, 3.0, 3.0, 1.0, 0.0, 1.0, 7.0, 5.0, 3.0, 1.0, 1.0,
                9.0, 5.0, 5.0, 1.0, 1.0, 7.0, 5.0, 3.0, 0.0, 0.0, 1.0, 1.0, 1.0, 0.0,
            ],
        );
        let mut collections: Collections = Collections::new();
        ...
        println!(
            "ookay collection: {}",
            collections.get("contours/01").unwrap().0.len()
        );
    ...
    fn vec_f64_to_coordinate_f32_all_the_things() {
        assert_eq!(
            Coordinate { x: 0.0, y: 1.1 },
            vec_f64_to_coordinate_f32(&vec![0.0_f64, 1.1_f64]).unwrap()
        );
    ...
    fn from_geojsonfeature_for_cratefeature_works_for_empty_multipolygon() {
        let geojsonfeature = Feature {
            ...
            geometry: Some(Geometry {
                bbox: None,
                value: MultiPolygon(vec![
                    // one multipolygon consists of n polygons
                    vec![
                        // one polygon with one or more linear rings
                        vec![
                            // one linear ring with n points, denoting the polygon surface
                            vec![0.0, 1.1],
                            vec![1.1, 2.2],
                            vec![2.2, 0.0],
                        ],
        ...
        let cratefeature: anyhow::Result<CrateFeature> =
            try_from_geojson_feature_for_crate_feature(geojsonfeature);
    ...
    fn collections_with_layers(
        layer_names: Vec<&str>, /*, add_features: bool*/
    ...
    fn fill_contour_layers_does_not_panic_if_no_contours_but_returns_err() {
        ...
        let res = fill_contour_layers(
            vec!["foo".to_string(), "contours/1".to_string()],
            &mut layers,
        );
    ...
    fn fill_contour_layers_copies_all_features_from_contours_to_contours_1() {
        ...
        for i in 0..=1 {
            assert_eq!(
                contours_features.get(i).unwrap().geometry,
                contours_1_features.get(i).unwrap().geometry
            );
        }
    ...
    fn fill_contour_layers_copies_only_every_fifth_feature_from_contours_to_contours_5() {
        ...
        assert_eq!(
            contours_features.get(0).unwrap().geometry,
            contours_5_features.get(0).unwrap().geometry
        );
        assert_eq!(
            contours_features.get(5).unwrap().geometry,
            contours_5_features.get(1).unwrap().geometry
        );
        assert_eq!(
            contours_features.get(10).unwrap().geometry,
            contours_5_features.get(2).unwrap().geometry
        );
    }
    ...
    fn build_lod_vector_tiles_creates_directories() {
        with_input_and_output_paths(|_, output_path| {
            ...
            let res =
                build_lod_vector_tiles(&mut layers, NonZeroUsize::new(4096).unwrap(), 2, &lod_path);
    ...
    fn create_tile_returns_tile_with_features() {
        ...
        foo_features.push(CrateFeature {
            geometry: point_on_tile,
            properties: HashMap::new(),
        });
        let point_off_tile = geo::Geometry::Point(geo::Point(Coordinate {
            x: 5000.0,
            y: 5000.0,
        }));
        foo_features.push(CrateFeature {
            geometry: point_off_tile,
            properties: HashMap::new(),
        });
    ...
    #[case(1024, 4096, 2)]
    #[case(2048, 4096, 3)]
    fn calc_max_lod_returns_good_values(
        #[case] world_size: usize,
        #[case] tile_size: usize,
        #[case] expected_lod: usize,
    ) {
...
pub fn try_from_geojson_feature_for_crate_feature(value: Feature) -> anyhow::Result<CrateFeature> {
    match value.geometry {
        Some(g) => try_from_geojson_value_for_geo_geometry(g.value).map(|geo| CrateFeature {
            geometry: geo,
            properties: HashMap::new(),
        }),
...
fn vec_f64_to_coordinate_f32(point: &Vec<f64>) -> anyhow::Result<Coordinate<f32>> {
    if point.len() < 2 {
        Err(anyhow::Error::msg(
            "vector is no coordinate pair: less than 2 values",
        ))
    } else {
        match (
            point.first().map(|x| x.to_f32()),
            point.get(1).map(|x| x.to_f32()),
        ) {
...
fn try_from_geojson_value_for_geo_geometry(value: Value) -> anyhow::Result<Geometry<f32>> {
    match value {
        Value::Point(pt) => vec_f64_to_coordinate_f32(&pt).map(|c| Point(geo::Point(c))),
        Value::MultiPoint(_) => Ok(geo::Geometry::MultiPoint(geo::MultiPoint(vec![
            geo::Point(Coordinate { x: 0.0, y: 1.1 }),
            geo::Point(Coordinate { x: 1.1, y: 2.2 }),
        ]))),
        Value::LineString(_) => {
            todo!()
        }
        ...
        Value::MultiPolygon(mp) => {
            let poly_results: anyhow::Result<Vec<geo::Polygon<f32>>> = mp
                .iter()
                .map(|poly: &PolygonType| {
                    // poly consists of linestrings:
                    let linestring_results: anyhow::Result<Vec<LineString<f32>>> = poly
                        .iter()
                        .map(|line| {
                            let res: anyhow::Result<Vec<Coordinate<f32>>> =
                                line.iter().map(vec_f64_to_coordinate_f32).collect();
                            res.map(LineString)
                        })
                        .collect();
                    linestring_results.map(|linestrings| {
                        geo::Polygon::new(linestrings.first().unwrap().clone(), vec![])
                    })
                })
                .collect();
...
impl MapboxVectorTiles {
    ...
    pub fn exec(&self, input_path: &Path, output_path: &Path) -> anyhow::Result<()> {
        ...
        crate::tilejson::write(
            ...
            "Mapbox Vector",
            &layer_names,
            "https://localhost/".to_string().add("{z}/{x}/{y}.pbf"),
        )?;
...
fn calc_max_lod(world_size: NonZeroUsize, tile_size: usize) -> anyhow::Result<usize> {
    ...
    let tile_size_f64 = tile_size.to_f64().ok_or(anyhow::Error::msg(format!(
        "could not convert {} to f64",
        tile_size
    )))?;
    let world_size_f64 = (world_size)
        .get()
        .to_f64()
    ...
    (world_size_f64 * 10.0_f64 / tile_size_f64)
        .max(1.0_f64)
        .log2()
        .ceil()
        .to_usize()
        .ok_or(anyhow::Error::msg(
            "could not convert to usize. Negative value?",
        ))
}
...
fn build_contours(
    ...
    let expand_by_cell_size = |(a, b): &(f32, f32)| -> (f32, f32) {
        (
            a * cell_size,
            world_size.get().to_f32().unwrap() - (b * cell_size),
        )
    };
    ...
    let min_elevation = dem64
        .iter()
        .filter(|x| *x != &no_data_value)
        .min_by(cmp)
        .ok_or(anyhow::Error::msg("no values in DEM raster"))?;
    let max_elevation = dem64
        .iter()
        .filter(|x| *x != &no_data_value)
        .max_by(cmp)
        .ok_or(anyhow::Error::msg("no values in DEM raster"))?;
    ...
    let thresholds: Vec<f64> = (min_elevation.to_i64().unwrap()..=max_elevation.to_i64().unwrap())
        .map(|f| f.to_f64().unwrap())
        .collect();

    println!(
        "contour builder: dimensions {} by {}",
        dem.dimensions().0 as u32,
        dem.dimensions().1 as u32
    );
    println!(
        "contour builder: elevation min {}, max {}, offset {}",
        min_elevation, max_elevation, elevation_offset
    );

    let res = builder
        .contours(&dem64, &thresholds)
        .map(|features: Vec<Feature>| {
            let mut cfs: Vec<CrateFeature> = features
                .into_iter()
                .filter_map(|f| try_from_geojson_feature_for_crate_feature(f).ok())
                .collect();

            cfs.iter_mut().for_each(|feature| {
                feature.map_coords_inplace(expand_by_cell_size);
            });

            collections.insert(String::from("contours/01"), FeatureCollection(cfs));
        });
    ...
    while let Ok(lod) = projection_lod {
        ...
        projection
            ...
            .for_each(|(name, collection)| {
                ...
                match name.as_str() {
                    "bunker" | "chapel" | "church" | "cross" | "fuelstation" | "lighthouse"
                    | "rock" | "shipwreck" | "transmitter" | "watertower" | "fortress"
                    | "fountain" | "view-tower" | "quay" | "hospital" | "busstop" | "stack"
                    | "ruin" | "tourism" | "powersolar" | "powerwave" | "powerwind" | "tree"
                    | "bush" => {}
                    "mount" => simplify_mounts(collection, 1000.0),
                    "railway" | "powerline" => collection.simplify(1.0),
                    "house" => collection.remove_empty(0.0, 70.0),
                    "contours" => {
                        collection.simplify(5.0);
                    ...
                    "roads/main_road" | "roads/road" | "roads/track" | "roads/trail" => {
                        collection.simplify(2.0)
                    }
                    "roads/main_road-bridge"
                    | "roads/road-bridge"
                    | "roads/track-bridge"
                    | "roads/trail-bridge" => {}
                    _ => {
        ...
        let layer_settings_path =
            Path::new("./resources/default_layer_settings.json").to_path_buf();
        let mut lod_layer_names = find_lod_layers(
            projection.get_collections_mut(),
            lod,
            &LayerSettingsFile::from_path(layer_settings_path),
        )?;
        if is_max_lod {
            // TODO special rule that should live somewhere else
            lod_layer_names = projection.get_collections().0.keys().cloned().collect()
        }
        let mut lod_layers: HashMap<String, FeatureCollection> = projection
            .get_collections()
            .iter()
            .filter(|c| lod_layer_names.contains(c.0))
        ...
        fill_contour_layers(lod_layer_names, projection.get_collections_mut()).unwrap_or_else(
            |e| {
        ...
        build_lod_vector_tiles(&mut lod_layers, world_size, lod, &lod_dir).unwrap_or_else(|err| {
            println!(
                "error when generating vector tiles for lod {}: {}",
                lod, err
            );
        });
...
fn build_lod_vector_tiles(
    ...
    println!(
        "build_lod_vector_tiles with {} collections, worldsize {} and lod {} into {}",
        collections.len(),
        world_size,
        lod,
        lod_dir.to_str().unwrap_or("WAT")
    );

    fn ensure_directory(dir: &PathBuf) -> Result<(), Error> {
        if !dir.is_dir() {
            std::fs::create_dir(dir).map_err(Error::new)
        } else {
            Ok(())
        }
    }

    let tiles_per_dimension: u16 = 2_u16.pow(lod as u32);
    ...
    for col in 0..tiles_per_dimension {
        ...
        for row in 0..tiles_per_dimension {
            ...
            let file_path = file_path_buf
                .to_str()
                .ok_or(Error::msg("couldnt create filename. wat."))?;
...
fn fill_contour_layers(
    lod_layer_names: Vec<String>,
    collections: &mut HashMap<String, FeatureCollection>,
) -> anyhow::Result<()> {
    ...
    let contours_names: Vec<(String, usize)> = lod_layer_names
        .iter()
        .filter_map(|name| {
            name.strip_prefix("contours/")
                .and_then(|num_str| {
